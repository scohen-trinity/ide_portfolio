...
function Experience() {
    const experience_list = [
      { title: 'SDE I - Frontend and Mobile', tenure: 'October 2025 - Present', img: `twitch.png`, company: 'Twitch', description: 'While at my current position at Twitch, I have built cutting edge engagement features across platforms. I recently created and launched [Weekly Rewards](https://help.twitch.tv/s/article/weekly-rewards), a mechanism to reward users for visiting for a fraction of a week, without punishing them for missing a single day due to life events.'},
      { title: 'Junior Full Stack Software Engineer', tenure: 'Jan 2025 - October 2025', img: `econtrols.jpg`, company: 'Econtrols', description: 'In my position as a Junior Full Stack Engineer I worked with Angular, Typescript, HTML as my primary frontend technologies and C#, .NET, and Azure Cloud on the backend.'},
              ...
              {experience_list.map(job => (
                <div className='job-block' key={ job.title + job.company }>
                  ...
                  <div className='job-description'>
                    <h3>{ job.company }</h3>
                    <p>{ renderDescription(job.description) }</p>
                  </div>
                </div>
              ))}
            </div>
